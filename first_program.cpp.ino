...
int led = 9; /*сетваме кой порт ще използваме*/
...
{
  pinMode(led, OUTPUT); /*казжаме на че ще използваме изход*/
  ...
  {
    analogWrite(led, light);/*в случая използваме променливата която върти цикъла за стойноста която ще свети светлината или ще се въртят оборотите;*/
  ...
  {
    analogWrite(led, light);/*сетваме от колко ще започне да се намалят стойностите*/
